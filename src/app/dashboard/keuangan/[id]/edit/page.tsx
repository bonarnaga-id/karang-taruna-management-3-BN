...
import { Save, X, Calendar, FileText, Wallet, Tag, User, Clock } from "lucide-react";
...
export default function EditKeuanganPage({
  ...
  const fetchTransaction = useCallback(() => {
    fetch(`/api/finance/${resolvedParams.id}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.data) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form || !form.amount || !form.category) return;
    setSubmitting(true);
    setError(null);

    try {
      const res = await fetch(`/api/finance/${resolvedParams.id}`, {
        method: "PUT",
  ...
  if (!form || !transaction) {
    return (
      <div className="animate-fade-in text-center py-16">
        ...
        <h3 className="text-lg font-semibold text-gray-600">Transaksi tidak ditemukan</h3>
        <Link href="/dashboard/keuangan" className="text-blue-600 hover:text-blue-700 font-medium">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            {transaction.type === "pemasukan" ? "Pemasukan" : "Pengeluaran"} — {transaction.category}
          </p>
        </div>
        <Link
